...
class CAVI(object):
    ...
    def update_prior(self):
        ...
        self.d = 10 * np.ones((self.N, self.A, self.O, self.Z))
        # parameters of p(rho|e, f) for (n) LTE & WiFi agents
        self.e = 0.1 * np.ones(self.N)
        self.f = 10 * np.ones(self.N)
    ...
    def init_q_param(self):
        ...
        self.a = 0.1 * np.ones((self.N, self.A, self.O, self.Z, self.Z))
        self.b = 10 * np.ones((self.N, self.A, self.O, self.Z, self.Z))
        
        # parameters of q(rho|g, h) for each (n) agent
        self.g = 0.1 * np.ones((self.N, self.Z))
        self.h = 10 * np.ones((self.N, self.Z))
    ...
    def calc_ELBO(self):
        ...
        # (2) E[lnp(rho| e, f)]
        t1 = (self.e - 1)[..., np.newaxis] * dglnh
        t2 = self.f[..., np.newaxis] * (self.g / self.h)
        prho = t1 - t2
        lowerbound += prho.sum()
        
        # (3) E[lnp(u| rho)]
        pu = dglnh + ((self.g / self.h) - 1) * d2d12_u
        lowerbound += pu.sum()
        
        # (4) E[lnp(V| alpha)]
        pv = dalnb + ((self.a / self.b) - 1) * d2d12_V
        lowerbound += pv.sum()
    ...
    def update_u(self):
        ...
        self.mu = (self.mu / self.ep) + (self.g / self.h)
    ...
    def update_v(self):
        ...
        self.lambda_ = (self.lambda_ / self.ep) + (self.a / self.b)
    ...
    def update_alpha(self):
        self.a = self.c[..., None] + np.ones(self.a.shape)
        
        d2d12 = digamma(self.lambda_) - digamma(self.sigma + self.lambda_)
        self.b = self.d[..., None] - d2d12
        
        
    def update_rho(self):
        self.g = self.e[..., None] + np.ones(self.g.shape)
        
        d2d12 = digamma(self.mu) - digamma(self.delta + self.mu)
        self.h = self.f[..., None] - d2d12
